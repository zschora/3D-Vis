...
// CreateTestMesh создаёт тетраэдр
...
func NewApplication(width, height int) *Application {
    ...
    app := &Application{
        width:    width,
        height:   height,
        renderer: NewRenderer(NewCamera(1000, math.Pi / 4, math.Pi / 4, 500)),  // Камера
        scenes:   make([]*Scene, 0),
    }

    // Создаем сцену и добавляем в неё тестовый меш
    sc := NewScene()
    sc.AddMesh(CreateCube())  // Добавляем меш в сцену
    app.scenes = append(app.scenes, sc)  // Добавляем сцену в приложение
...
func (app *Application) Update() {
    ...
    //app.renderer.MyCamera.ScaleLinear (deltaTime*100)
}

func (app *Application) Render() {
    rl.BeginDrawing()
    rl.ClearBackground(rl.LightGray) // Белый фон
